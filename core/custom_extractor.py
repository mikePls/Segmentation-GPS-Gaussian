...
class UnetExtractor(nn.Module):
    def __init__(self, in_channel=3, encoder_dim=[64, 96, 128], norm_fn='group'):
        ...
        self.att1 = AttentionGate(F_g=encoder_dim[0], F_l=encoder_dim[0], F_int=encoder_dim[0]//2)
        ...
        self.att2 = AttentionGate(F_g=encoder_dim[1], F_l=encoder_dim[1], F_int=encoder_dim[1]//2)
        ...
        self.att3 = AttentionGate(F_g=encoder_dim[2], F_l=encoder_dim[2], F_int=encoder_dim[2]//2)

    def forward(self, x):
        x = self.in_ds(x)
        x1 = self.res1(x)
        x1 = self.att1(x1, x1)  # Apply attention gate

        x2 = self.res2(x1)
        x2 = self.att2(x2, x2)  # Apply attention gate

        x3 = self.res3(x2)
        x3 = self.att3(x3, x3)  # Apply attention gate
